...
| **2** | Paint → typed Renderer (net captures the full primitive stream) | ⏳ **in progress** — hoists (D48) + `Renderer` pilot: label/button (D49), then checkbox, the shared `paintField` helper, and both `paintArrow` overloads (unlocked by D51), extracted zero-diff; remaining branches follow slice by slice (golden-guarded first; D50 shared-helper gate) |
