...
| 4 | Scrollbar/spinbox/combobox part tokens | 15 tokens (`up down left right *track vknob hknob corner icon text modal`) | **Cut 6** | Internal-only; hit-testing (`findScroll`/`findComponent`) owns them — event/input territory, not Cut 5. Internal enum when the mouse-state fields are typed. |
| 5 | Dialog part tokens | 9 (`"header"` + compass `":n" ":s" ":e" ":w" ":nw" ":ne" ":sw" ":se"`) | **Cut 6** | Same owner as #4. The `:` prefix here marks part tokens, *not* model keys — any typing pass must not merge them with #8. |
...
| 9 | Event names | 11 (`init focusgained focuslost action insert remove caret perform expand collapse menushown` — exactly the `"method"`-typed table rows; `init` bypasses `invoke`) | **3c** | Public-facing (DTD method attributes) — public constants candidate alongside a typed listener API. |
