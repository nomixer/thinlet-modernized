...
public class Thinlet extends Container implements Runnable, Serializable {
    ...
    private void doLayout(Object component) {
        ...
        if (is(classname, "combobox")) {
            ...
            layoutField(component, 0, (is(classname, "passwordfield")), 0);
            ...
            if (wrap) {
                ...
                chars = getChars(this, component, text, true, bounds.width - 4, bounds.height);
                if (chars == null) { // need scrollbars
                    chars = getChars(this, component, text, true, bounds.width - block - 4, 0);
                }
            } else {
                chars = getChars(this, component, text, false, 0, 0);
            }
            ...
            layoutScroll(
                    component,
                    width + 2,
                    height - fm.getLeading() + 2,
                    0,
                    0,
                    0,
                    0,
                    getBoolean(component, "border", true),
                    0);
            ...
            int cx = (is(placement, "left")) ? (tabsize + 1) : 2;
            int cy = (is(placement, "top")) ? (tabsize + 1) : 2;
            ...
            for (Object tab = get(component, ":comp"); tab != null; tab = get(tab, ":next")) {
                ...
                if (!stacked) {
                    if (horizontal) {
                        if (is(placement, "bottom")) {
                            r.y = bounds.height - tabsize;
                        }
                        r.height = tabsize;
                    } else {
                        if (is(placement, "right")) {
            ...
            boolean border = (is(classname, "panel")) && getBoolean(component, "border", false);
            ...
            if (scrollable) { // set scrollpane areas
                if (is(classname, "panel")) {
                    ...
                    scrollable =
                            layoutScroll(component, contentwidth, contentheight, 3 + titleheight, 3, 3, 3, true, 0);
                }
            }
            ...
            for (Object comp = get(component, ":comp"); comp != null; comp = get(comp, ":next")) {
                ...
                } else if ((!is(iclass, ":combolist")) && (!is(iclass, ":popup"))) {
            ...
            Rectangle bounds = getRectangle(component, "bounds");
            boolean horizontal = (!is(get(component, "orientation"), "vertical"));
            ...
            if (visible1) {
                setRectangle(
                        ...
                        0,
                        0,
                        horizontal ? divider : bounds.width,
    ...
    static char[] getChars(Thinlet t, Object component, String text, boolean wrap, int width, int height) {
    ...
    private Object popupMenu(Object component) {
        ...
        if ((selected == null) || (!is(getClass(selected), "menu"))) {
            return null;
        }
        ...
        boolean menubar = (is(getClass(component), "menubar"));
        if (menubar) {
        ...
        popup(
                ...
                popup,
                menubar ? 'D' : 'R',
                menubar ? (menux + menubounds.x) : menux,
                menuy + menubounds.y,
                menubar ? menubounds.width : menuwidth,
                menubar ? menuheight : menubounds.height,
                menubar ? 1 : 3);
        return popup;
    }
    ...
    private void popup(Object menu, Object popup, char direction, int x, int y, int width, int height, int offset) {
        ...
        for (Object item = get(menu, ":comp"); item != null; item = get(item, ":next")) {
            ...
            Dimension d = (is(itemclass, "separator")) ? new Dimension(1, 1) : getSize(item, 8, 4);
            if (is(itemclass, "checkboxmenuitem")) {
                d.width = d.width + block + 3;
                d.height = Math.max(block, d.height);
            } else if (is(itemclass, "menu")) {
                d.width += block;
            }
        ...
        if (direction == 'R') {
            ...
            y += ((direction == 'U') ? (topspace || !bottomspace) : (!bottomspace && topspace))
                    ? (offset - ph)
                    : (height - offset);
        }
        setRectangle(
                ...
                Math.max(0, Math.min(y, desktop.height - ph)),
                pw,
                ph);
        repaint(popup);
    }
    ...
    private void showTip() {
        ...
        if ((is(classname, "tabbedpane")) || (is(classname, "menubar")) || (is(classname, ":popup"))) {
            if (insidepart != null) {
    ...
    private boolean layoutScroll(
        ...
        if (hneed) {
            setRectangle(
                    ...
                    ":horizontal",
                    left,
                    bounds.height - bottom - block - 1,
                    bounds.width - left - right - (vneed ? block : 0),
                    block + 1);
        } else {
        ...
        if (vneed) {
            setRectangle(
                    component,
                    ":vertical",
                    bounds.width - right - block - 1,
                    top,
                    block + 1,
                    bounds.height - top - bottom - (hneed ? block : 0));
        } else {
    ...
    private Dimension getPreferredSize(Object component) {
        ...
        if (is(classname, "tabbedpane")) {
            String placement = getString(component, "placement", "top");
            boolean horizontal = ((!is(placement, "left")) && (!is(placement, "right")));
            ...
            return new Dimension(
                    contentwidth + (horizontal ? 4 : (tabsize + 3)), contentheight + (horizontal ? (tabsize + 3) : 4));
        }
        if ((is(classname, "panel")) || (is(classname, "dialog"))) {
            ...
            for (Object comp = get(component, ":comp"); comp != null; comp = get(comp, ":next")) {
                String iclass = getClass(comp);
                if ((!is(iclass, "dialog")) && (!is(iclass, ":popup")) && (!is(iclass, ":combolist"))) {
                    Dimension d = getPreferredSize(comp);
                    size.width = Math.max(d.width, size.width);
                    size.height = Math.max(d.height, size.height);
                }
        ...
        if ((is(classname, "list")) || (is(classname, "table")) || (is(classname, "tree"))) {
            return new Dimension(76 + 2 + block, 76 + 2 + block);
        }
        if (is(classname, "separator")) {
        ...
        if (horizontal) {
            ...
            for (int i = 0; i < width; i += block) {
                g.drawImage(
                        ...
                        0,
                        0,
                        Math.min(block, width - i) + evm,
                        Math.min(block, height) + evm,
                        null);
            }
        } else {
            if (width > block) {
            ...
            for (int i = 0; i < height; i += block) {
                g.drawImage(
                        vgradient,
                        (width > block) ? (x + width - block) : x,
                        y + i,
                        x + width + evm,
                        y + Math.min(i + block, height) + evm,
                        0,
                        0,
    ...
    void paint(Object component, Graphics g, IconTextSpec s) {
        ...
        int cx = x;
        if (is(alignment, "center")) {
            cx += (width - tw - iw) / 2;
        } else if (is(alignment, "right")) {
            cx += width - tw - iw;
        }
    ...
    protected void processEvent(AWTEvent e) {
        ...
        if ((id == MouseEvent.MOUSE_ENTERED)
            ...
            boolean popuptrigger =
                    (id == MouseEvent.MOUSE_PRESSED) && me.isMetaDown(); // isPopupTrigger is platform dependent
            if (id == MouseEvent.MOUSE_ENTERED) {
                ...
                if (popupowner != null) { // remove popup
                    String classname = getClass(mouseinside);
                    if ((popupowner != mouseinside) && (!is(classname, ":popup")) && (!is(classname, ":combolist"))) {
                        closeup();
                    }
                }
                ...
                if (wasin && !isin) {
                    ...
                    handleMouseEvent(
                            x, y, clickcount, shiftdown, controldown, popuptrigger, DRAG_EXITED, previnside, prevpart);
                }
                if (isin && !wasin) {
                    handleMouseEvent(
                            x,
            ...
            if (focusinside && ((popupowner != null) || (focusowner != null))) {
                ...
                if ((control == (id == KeyEvent.KEY_PRESSED))
                        && processKeyPress(
                                (popupowner != null) ? popupowner : focusowner,
                                ke.isShiftDown(),
                                ke.isControlDown(),
                                ke.getModifiers(),
                                control ? 0 : keychar,
                    ...
                    for (Object splitpane = focusowner; splitpane != null; splitpane = getParent(splitpane)) {
                        if (is(getClass(splitpane), "splitpane")) {
                            setFocus(splitpane);
                            repaint(splitpane);
                            ke.consume();
                            break; // middle
    ...
    private void checkLocation(Object component) {
        if (mouseinside == component) { // parameter added by scolebourne
            ...
            // 2005 passed mousex for the y argument; fixed in 0.2.x (D70) after
            // D68 proved the argument unconsumed on every MOUSE_ENTERED path.
            // Canary: InputQuirkPinsTest
            // #closingTheDropDownUnderTheCursorCommitsAndStaysConsistent.
            handleMouseEvent(mousex, mousey, 1, false, false, false, MouseEvent.MOUSE_ENTERED, mouseinside, insidepart);
        }
    }

    private boolean processKeyPress(
            Object component, boolean shiftdown, boolean controldown, int modifiers, int keychar, int keycode) {
        String classname = getClass(component);
        if (is(classname, "button")) {
            if (keychar == KeyEvent.VK_SPACE
                    || ((keycode == KeyEvent.VK_ENTER) && (is(get(component, "type"), "default")))
                    || ((keycode == KeyEvent.VK_ESCAPE)
                            && // ...
                            (is(get(component, "type"), "cancel")))) {
                // pressedkey = keychar;
                invoke(component, null, "action");
                repaint(component);
                return true;
            }
        } else if ((is(classname, "checkbox")) || (is(classname, "togglebutton"))) {
            if (keychar == KeyEvent.VK_SPACE) {
                changeCheck(this, component, true);
                repaint(component);
                return true;
            }
        } else if (is(classname, "combobox")) {
            ...
            if ((keycode == KeyEvent.VK_HOME) && !controldown) {
                ...
                return processField(component, shiftdown, controldown, modifiers, keychar, keycode, true, false, false);
            ...
            if (keycode == KeyEvent.VK_LEFT) {
                ...
                if ((get(lead, ":comp") != null) && getBoolean(lead, "expanded", true)) { // collapse
                    ...
                    selectItem(this, component, lead, true);
                    validate(component);
                    ...
                    if (parent != component) {
                        selectItem(this, component, parent, true);
                        setLead(this, component, lead, parent);
                        return true;
                    }
                }
            }
            // ? for interval mode select its all subnode or deselect all after
                ...
                if (node != null) {
                    if (getBoolean(lead, "expanded", true)) { // select its first subnode
                        selectItem(this, component, node, true);
                        setLead(this, component, lead, node);
                    } else { // expand
            ...
            if ((keycode == KeyEvent.VK_UP) || (keycode == KeyEvent.VK_DOWN)) {
                ...
                if ((keycode != KeyEvent.VK_ESCAPE) && getBoolean(selected, "enabled", true)) {
                    if ((selected != null) && (is(getClass(selected), "checkboxmenuitem"))) {
                        changeCheck(this, selected, false);
    ...
    static boolean changeCheck(Thinlet t, Object component, boolean box) {
        String group = t.getString(component, "group", null);
        if (group != null) {
            if (t.getBoolean(component, "selected", false)) {
                return false;
            }
            for (Object comp = get(t.getParent(component), ":comp"); comp != null; comp = get(comp, ":next")) {
                if (comp == component) {
                    t.setBoolean(component, "selected", true);
                } else if (group.equals(get(comp, "group")) && t.getBoolean(comp, "selected", false)) {
                    t.setBoolean(comp, "selected", false);
                    if (box) {
                        t.repaint(comp);
                    } // checkbox only
                }
            ...
            t.setBoolean(component, "selected", !t.getBoolean(component, "selected", false), false);
        }
        t.invoke(component, null, "action");
        return true;
    }
    ...
    private boolean processField(
        ...
        if (editable
            ...
            try {
                insert = (String)
                        getToolkit().getSystemClipboard().getContents(this).getTransferData(DataFlavor.stringFlavor);
            } catch (Exception exc) {
                insert = clipboard;
            }
     ...
     */
    private boolean changeField(
            Object component, String text, String insert, int movestart, int moveend, int start, int end) {
     ...
     */
    static void selectAll(Thinlet t, Object component, boolean selected, boolean recursive) {
        boolean changed = false;
        for (Object item = get(component, ":comp"); item != null; item = getNextItem(t, component, item, recursive)) {
            if (t.setBoolean(item, "selected", selected, false)) {
                t.repaint(component, null, item);
                changed = true;
            }
        }
     ...
     * @param recursive true for tree
     */
    static void selectItem(Thinlet t, Object component, Object row, boolean recursive) {
        boolean changed = false;
        for (Object item = get(component, ":comp"); item != null; item = getNextItem(t, component, item, recursive)) {
            if (t.setBoolean(item, "selected", (item == row), false)) {
                t.repaint(component, null, item);
                changed = true;
            }
        }
        set(component, ":anchor", null);
        if (changed) {
            t.invoke(component, row, "action");
        }
    }

    static void extend(Thinlet t, Object component, Object lead, Object row, boolean recursive) {
        ...
        for (Object item = get(component, ":comp"); // anchor - row
            ...
            if (t.setBoolean(item, "selected", (select != 'n'), false)) {
                t.repaint(component, null, item);
                changed = true;
            }
     ...
     */
    static void setLead(Thinlet t, Object component, Object oldlead, Object lead) {
        if (oldlead != lead) { // ?
            if (oldlead != null) {
                t.repaint(component, null, oldlead);
            }
            set(component, ":lead", lead);
            t.repaint(component, null, lead);

            Rectangle r = t.getRectangle(lead, "bounds");
            t.scrollToVisible(component, r.x, r.y, 0, r.height);
        }
    }
    ...
    private void handleMouseEvent(
        ...
        String classname = getClass(component);
        if ((is(classname, "button")) || (is(classname, "checkbox")) || (is(classname, "togglebutton"))) {
            if ((id == MouseEvent.MOUSE_ENTERED)
                    || (id == MouseEvent.MOUSE_EXITED)
                    || (id == MouseEvent.MOUSE_PRESSED)
                    || (id == MouseEvent.MOUSE_RELEASED)) {
                ...
                if ((is(classname, "button"))
                        ...
                        && (is(get(component, "type"), "link"))) {
                    setCursor(Cursor.getPredefinedCursor(
                            (id == MouseEvent.MOUSE_ENTERED) ? Cursor.HAND_CURSOR : Cursor.DEFAULT_CURSOR));
            ...
            processField(x, y, clickcount, id, component, false, (is(classname, "passwordfield")), 0, popuptrigger);
        } else if (is(classname, "textarea")) {
            if (!processScroll(x, y, id, component, part)) {
                processField(x, y, clickcount, id, component, true, false, 0, popuptrigger);
            }
        } else if (is(classname, "panel")) {
            ...
            if (!processScroll(x, y, id, component, part)) {
                if (((id == MouseEvent.MOUSE_PRESSED)
                    ...
                    for (Object item = get(component, ":comp"); item != null; ) {
                        ...
                        if (my < r.y + r.height) {
                            if (id == MouseEvent.MOUSE_DRAGGED) { // !!!
                                ...
                                if (mx < r.x) {
                                    if ((mx >= r.x - block) && (get(item, ":comp") != null)) {
                                        ...
                                        selectItem(this, component, item, true);
                                        setLead(this, component, get(component, ":lead"), item);
                                        setFocus(component);
                                        validate(component);
                            ...
                            if ((id != MouseEvent.MOUSE_DRAGGED) || !getBoolean(item, "selected", false)) {
                                ...
                                if (!popuptrigger || !getBoolean(item, "selected", false)) { // don't update selection
                                    select(this, component, item, (is(classname, "tree")), shiftdown, controldown);
                                    if (clickcount == 2) {
                        ...
                        item = getNextItem(this, component, item, (is(classname, "tree")));
                    }
                }
            }
        } else if (is(classname, "menubar")) {
            ...
            if (((id == MouseEvent.MOUSE_ENTERED) || (id == MouseEvent.MOUSE_EXITED))
                ...
                if ((part != insidepart)
                        && ((insidepart == null)
                                || ((insidepart instanceof Object[]) && (!is(getClass(insidepart), "menu"))))) {
        ...
        } else if ((is(classname, ":popup")) || (is(classname, "popupmenu"))) {
            if (((id == MouseEvent.MOUSE_ENTERED) || (id == DRAG_ENTERED))
                    && (part != null)
                    && getBoolean(part, "enabled", true)) {
                set(component, "selected", part);
            ...
            if (is(part, ":close")) {
                ...
                if (id == MouseEvent.MOUSE_PRESSED) {
                    ...
                    repaint(component);
                    Rectangle bounds = getRectangle(component, "bounds");
                    if ((is(part, ":nw")) || (is(part, ":n")) || (is(part, ":ne"))) {
                        bounds.y += y - referencey;
                        bounds.height -= y - referencey;
                    }
                    if ((is(part, ":ne")) || (is(part, ":e")) || (is(part, ":se"))) {
                        bounds.width += x - referencex;
                    }
                    if ((is(part, ":sw")) || (is(part, ":s")) || (is(part, ":se"))) {
                        bounds.height += y - referencey;
                    }
                    if ((is(part, ":nw")) || (is(part, ":w")) || (is(part, ":sw"))) {
    ...
    static void select(
            Thinlet t, Object component, Object row, boolean recursive, boolean shiftdown, boolean controldown) {
        String selection = t.getString(component, "selection", "single");
        Object lead = null;
        if (shiftdown && (!is(selection, "single")) && ((lead = get(component, ":lead")) != null)) {
            extend(t, component, lead, row, recursive);
        } else {
            if (controldown && (is(selection, "multiple"))) {
                t.setBoolean(row, "selected", !t.getBoolean(row, "selected", false), false);
                t.repaint(component, null, row);
                t.invoke(component, row, "action");
                set(component, ":anchor", null);
            } else if (controldown && t.getBoolean(row, "selected", false)) {
                for (Object item = row; item != null; item = getNextItem(t, component, item, recursive)) {
                    if (t.setBoolean(item, "selected", false, false)) {
                        t.repaint(component, null, item);
                    }
                }
                t.invoke(component, row, "action");
        ...
        setLead(t, component, (lead != null) ? lead : get(component, ":lead"), row);
    }
     ...
     */
    static Object getNextItem(Thinlet t, Object component, Object item, boolean recursive) {
        if (!recursive) {
            return get(item, ":next");
        }
        Object next = get(item, ":comp");
        if ((next == null) || !t.getBoolean(item, "expanded", true)) {
            while ((item != component) && ((next = get(item, ":next")) == null)) {
                item = t.getParent(item);
            }
        }
        return next;
    }
    ...
    private void processField(
        ...
        if (id == MouseEvent.MOUSE_PRESSED) {
            ...
            if (popuptrigger) {
                int start = getInteger(component, "start", 0);
                int end = getInteger(component, "end", 0);
                if ((caretstart >= Math.min(start, end))
                        && // inside selected text
    ...
    private boolean processScroll(int x, int y, int id, Object component, Object part) {
        ...
        } else if ((is(part, "vknob")) || (is(part, "hknob"))) {
            if (id == MouseEvent.MOUSE_PRESSED) {
                Rectangle port = getRectangle(component, ":port");
                Rectangle view = getRectangle(component, ":view");
                if (is(part, "hknob")) {
    ...
    private boolean processScroll(Object component, Object part) {
        Rectangle view = getRectangle(component, ":view");
        Rectangle port = ((is(part, "left")) || (is(part, "up"))) ? null : getRectangle(component, ":port");
        int dx = 0;
        int dy = 0;
        if (is(part, "left")) {
            dx = -10;
        } else if (is(part, "lefttrack")) {
            dx = -port.width;
        } else if (is(part, "right")) {
            dx = 10;
        } else if (is(part, "righttrack")) {
            dx = port.width;
        } else if (is(part, "up")) {
            dy = -10;
        } else if (is(part, "uptrack")) {
            dy = -port.height;
        } else if (is(part, "down")) {
            dy = 10;
        } else if (is(part, "downtrack")) {
            dy = port.height;
        }
        if (dx != 0) {
        ...
        return (((is(part, "left")) || (is(part, "lefttrack"))) && (view.x > 0))
                || (((is(part, "right")) || (is(part, "righttrack"))) && (view.x < view.width - port.width))
                || (((is(part, "up")) || (is(part, "uptrack"))) && (view.y > 0))
                || (((is(part, "down")) || (is(part, "downtrack"))) && (view.y < view.height - port.height));
    }
    ...
    private void invokeImpl(Object method, Object part) {
        ...
        if (args != null)
            for (int i = 0; i < args.length; i++) {
                ...
                if (is(target, "thinlet")) {
                    args[i] = this;
                } else if ((is(target, "constant"))) { // constant value
                    args[i] = data[2 + 3 * i + 1];
                } else {
                    if (is(target, "item")) {
                        target = part;
    ...
    private boolean findComponent(Object component, int x, int y) {
        ...
        if (is(classname, "combobox")) {
        ...
        } else if ((is(classname, "panel")) || (is(classname, "desktop")) || (is(classname, "dialog"))) {
            if (is(classname, "dialog")) {
            ...
            if ((insidepart == null) && !findScroll(this, component, x, y)) {
                ...
                for (Object comp = get(component, ":comp"); comp != null; comp = get(comp, ":next")) {
                    ...
                    if ((is(classname, "desktop")) && getBoolean(comp, "modal", false)) {
                        insidepart = "modal";
                        break;
                    } // && dialog
                }
    ...
    private void repaint(Object component, Object classname, Object part) {
        ...
        // else if (is(classname, "splitpane")) {}
                ...
                || // tab
                (is(classname, "menubar"))
                || (is(classname, ":popup"))) { // menuitem
            Rectangle r = getRectangle(part, "bounds");
            repaint(component, b.x + r.x, b.y + r.y, (is(classname, ":popup")) ? b.width : r.width, r.height);
        }
        // classname: ":combolist" "textarea" "list" "table" "tree"
        else if ((is(part, "left")) || (is(part, "right"))) { // horizontal scrollbar button
            Rectangle r = getRectangle(component, ":horizontal");
            repaint(component, b.x + ((is(part, "left")) ? r.x : (r.x + r.width - block)), b.y + r.y, block, r.height);
        } else if ((is(part, "up")) || (is(part, "down"))) { // vertical scrollbar button
            Rectangle r = getRectangle(component, ":vertical");
            repaint(component, b.x + r.x, b.y + ((is(part, "up")) ? r.y : (r.y + r.height - block)), r.width, block);
        } else if ((is(part, "text")) || (is(part, "horizontal")) || (is(part, "vertical"))) {
    ...
    private boolean setNextFocusable(Object current, boolean outgo) {
        ...
        for (Object next = null, component = current; true; component = next) {
            ...
            while (next == null) { // find the next of the parents, or the topmost
                ...
                if ((component == content)
                        || ((is(getClass(component), "dialog")) && (!outgo || getBoolean(component, "modal", false)))) {
     ...
     */
    private Object getPreviousFocusable(
            Object component, Object block, boolean start, boolean upward, boolean backward, boolean outgo) {
        Object previous = null;
        if ((component != null) && (component != block)) {
            boolean go = ((!is(getClass(component), "dialog")) || (outgo && !getBoolean(component, "modal", false)));
            if (!start && !upward && go) {
    ...
    private boolean isFocusable(Object component, boolean forced) {
        String classname = getClass(component);
        if ((is(classname, "button"))
                || (is(classname, "checkbox"))
                || (is(classname, "togglebutton"))
                || (is(classname, "combobox"))
                || (is(classname, "textfield"))
                || (is(classname, "passwordfield"))
                || (is(classname, "textarea"))
                || (is(classname, "spinbox"))
                || (is(classname, "slider"))
                || (is(classname, "list"))
                || (is(classname, "table"))
                || (is(classname, "tree"))
                || (is(classname, "tabbedpane"))
                || (forced && (is(classname, "splitpane")))) {
            for (Object comp = component; comp != null; ) {
                ...
                if ((is(getClass(comp), "tab")) && (getItem(parent, getInteger(parent, "selected", 0)) != comp)) {
                    return false;
                }
                comp = parent;
            }
    ...
    public int getSelectedIndex(Object component) {
        String classname = getClass(component);
        if ((is(classname, "combobox")) || (is(classname, "tabbedpane"))) {
            return getInteger(component, "selected", (is(classname, "combobox")) ? -1 : 0);
        }
        if ((is(classname, "list")) || (is(classname, "table")) || (is(classname, "tree"))) {
    ...
    public Object getSelectedItem(Object component) {
        String classname = getClass(component);
        if ((is(classname, "combobox")) || (is(classname, "tabbedpane"))) {
            int index = getInteger(component, "selected", (is(classname, "combobox")) ? -1 : 0);
            return (index != -1) ? getItemImpl(component, ":comp", index) : null;
        }
        if ((is(classname, "list")) || (is(classname, "table")) || (is(classname, "tree"))) {
            for (Object item = findNextItem(this, component, classname, null);
                    item != null;
                    item = findNextItem(this, component, classname, item)) {
                if (getBoolean(item, "selected", false)) {
                    return item;
                }
            }
     ...
     */
    static Object findNextItem(Thinlet t, Object component, String classname, Object item) {
        if (item == null) { // first item
            return get(component, ":comp");
        } else if (is(classname, "tree")) { // next tree node
            Object next = get(item, ":comp");
            if ((next == null) || !t.getBoolean(item, "expanded", true)) { // no subnode or collapsed
                while ((item != component) && ((next = get(item, ":next")) == null)) {
                    item = t.getParent(item); // next node of in backward path
                }
            }
            return next;
    ...
    public void remove(Object component) {
        ...
        if ((is(classname, "popupmenu")) || (is(classname, "header"))) {
    ...
    public Object find(Object component, String name) {
        ...
        Object header = get(component, "header"); // if (is(classname, "table"))
    ...
    private boolean checkMnemonic(Object component, boolean parent, Object checked, int keycode, int modifiers) {
        if ((component == null)
                || !getBoolean(component, "visible", true)
        ...
        } else if (is(classname, "button")) {
            if (((modifiers == 0)
                            && (((keycode == KeyEvent.VK_ENTER) && (is(get(component, "type"), "default")))
                                    || ((keycode == KeyEvent.VK_ESCAPE) && (is(get(component, "type"), "cancel")))))
        ...
        } else if ((is(classname, "menuitem")) || (is(classname, "checkboxmenuitem"))) {
        ...
        // check subcomponents
        if ((is(classname, "panel"))
                || (is(classname, "desktop"))
                || (is(classname, "dialog"))
                || (is(classname, "splitpane"))
                || (is(classname, "menubar"))
                || (is(classname, "menu"))) {
            for (Object comp = get(component, ":comp"); comp != null; comp = get(comp, ":next")) {
        ...
        // check parent
        if (parent && ((!is(classname, "dialog")) || !getBoolean(component, "modal", false))) {
            if (checkMnemonic(
                    getParent(component), true, (is(classname, "tab")) ? checked : component, keycode, modifiers)) {
                return true;
            }
        }
        return false;
     ...
     * @throws java.lang.IllegalArgumentException
     */
    private Object parse(InputStream inputstream, char mode, Object handler) throws IOException {
                                ...
                                while (quote != (c = reader.read())) {
                                    if (c == '&') {
                                        ...
                                        if ("lt".equals(entity)) {
                                            ...
                                            text.append((char)
                                                    Integer.parseInt(entity.substring(hexa ? 2 : 1), hexa ? 16 : 10));
    ...
    static void addImpl(Object parent, Object component, int index) {
        ...
        if (((is(parentclass, "combobox")) && (is(classname, "choice")))
                || ((is(parentclass, "tabbedpane")) && (is(classname, "tab")))
                || ((is(parentclass, "list")) && (is(classname, "item")))
                || ((is(parentclass, "table")) && (is(classname, "row")))
                || ((is(parentclass, "header")) && (is(classname, "column")))
                || ((is(parentclass, "row")) && (is(classname, "cell")))
                || (((is(parentclass, "tree")) || (is(parentclass, "node"))) && (is(classname, "node")))
                || ((is(parentclass, "menubar")) && (is(classname, "menu")))
                || (((is(parentclass, "menu")) || (is(parentclass, "popupmenu")))
                        && ((is(classname, "menu"))
                                || (is(classname, "menuitem"))
                                || (is(classname, "checkboxmenuitem"))
                                || (is(classname, "separator"))))
                || (((is(parentclass, "panel"))
                                || (is(parentclass, "desktop"))
                                || (is(parentclass, "splitpane"))
                                || (is(parentclass, "dialog"))
                                || (is(parentclass, "tab")))
                        && instance(classname, "component")
                        && (!is(classname, "popupmenu")))) {
            insertItem(parent, ":comp", component, index);
            set(component, ":parent", parent);
        } else if (((is(parentclass, "table")) && (is(classname, "header")))
                || ((is(classname, "popupmenu")) && instance(parentclass, "component"))) {
        ...
        if (value != null) {
            ...
            try {
                ...
                while (st.hasMoreTokens()) {
                    ...
                    try {
                        modifiers = modifiers
                                | InputEvent.class.getField(token + "_MASK").getInt(null);
        ...
        for (int i = 0; i < n; i++) {
            ...
            if ("thinlet".equals(arg)) {
                ...
                if ("item".equals(compname)) {
                    ...
                    if (is(parentclass, "list")) {
                        classname = "item";
                    } else if (is(parentclass, "tree")) {
                        classname = "node";
                    } else if (is(parentclass, "table")) {
                        classname = "row";
                    } else if (is(parentclass, "combobox")) {
                        classname = "choice";
                    } else if (is(parentclass, "tabbedpane")) {
                        classname = "tab";
    ...
    private void update(Object component, String mode) {
        ...
        while (component != null) {
            ...
            if (is(mode, "paint")) { // || (firstpaint && (component == content))
                Rectangle bounds = getRectangle(component, "bounds");
                if (bounds == null) {
                    return;
                }
            ...
            } else if (is(classname, "menu")) {
                parent = get(parent, ":popup");
            } else if ((is(mode, "paint")) && (is(classname, "tabbedpane"))) {
