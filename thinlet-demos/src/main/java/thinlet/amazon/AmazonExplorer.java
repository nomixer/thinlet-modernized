...
public class AmazonExplorer extends Thinlet {
    ...
    private static final String[] DVD = {"+salesrank", "+titlerank"};
    private static final String[] ELECTRONICS = {"+pmrank", "+salesrank", "+titlerank", "+reviewrank"};
    ...
    private static final String[] HARDWARE = {"+psrank", "+salesrank", "+titlerank", "-titlerank"};
    private static final String[] PHOTO = {"+pmrank", "+salesrank", "+titlerank", "-titlerank"};
    private static final String[] SOFTWARE = {"+pmrank", "+salesrank", "+titlerank", "+price", "-price"};
    private static final String[] VHS = {"+psrank", "+salesrank", "+titlerank"};
    private static final String[] VIDEOGAMES = {"+pmrank", "+salesrank", "+titlerank", "+price", "-price"};
    ...
    public void checkDetails(Object details, int selectedtab) {
        if (selectedtab == 0) {
            return;
        } // first tab contains lite information
        ...
        if (sAsin != null) {
            ...
            if (!isErrorMessage(nProductInfo)) {
                ...
                loadList(
                        getDOMNode(nDetails, "Platforms", 0), "Platform", detailpanel, "platformtitle", "platformlist");
                loadList(getDOMNode(nDetails, "Features", 0), "Feature", detailpanel, "featuretitle", "featurelist");
    ...
    private void loadReview(Object reviewpanel, Object nReviews, int i) {
        ...
        if ((i >= 0) && (i < n)) {
            ...
            setString(find(reviewpanel, "comment"), "text", convertHTML(getDOMText(nCustomerReview, "Comment")));
        }
    }
    ...
    public void showWishlists(Object button) {
        try {
            ...
            loadWishlist(
                    ...
                    0,
                    listmania,
                    find(listmania, "resultlist"),
                    find(listmania, "listname"),
                    find(listmania, "prevlist"),
                    find(listmania, "nextlist"));
            addPage(listmania);
        } catch (Exception exc) {
            exc.printStackTrace();
        }
    ...
    public void previousWishlist(
            Object listmania, Object resultlist, Object listname, Object prevlist, Object nextlist) {
        int index = ((Integer) getProperty(listmania, "ListsIndex")).intValue();
        loadWishlist(getProperty(listmania, "Lists"), index - 1, listmania, resultlist, listname, prevlist, nextlist);
    }

    public void nextWishlist(Object listmania, Object resultlist, Object listname, Object prevlist, Object nextlist) {
        int index = ((Integer) getProperty(listmania, "ListsIndex")).intValue();
        loadWishlist(getProperty(listmania, "Lists"), index + 1, listmania, resultlist, listname, prevlist, nextlist);
    }
    ...
    public void yellowTheme() {
        setColors(0xeeeecc, 0x000000, 0xffffff, 0x999966, 0xb0b096, 0xededcb, 0xcccc99, 0xcc6600, 0xffcc66);
    }

    public void blueTheme() {
        setColors(0x6375d6, 0xffffff, 0x7f8fdd, 0xd6dff5, 0x9caae5, 0x666666, 0x003399, 0xff3333, 0x666666);
    }
    ...
    private Object getResponse(String path) {
        ...
        String url = "http://xml.amazon.com/onca/xml2?t=" + associatestag + "&dev-t=" + developertoken + "&" + path
                + "&f=xml";
    ...
    private static int getNumber(String text) {
        try {
            ...
            text = text + "00";
            if (dot != -1) {
                text = text.substring(0, dot) + text.substring(dot + 1, Math.min(dot + 3, text.length()));
            }
