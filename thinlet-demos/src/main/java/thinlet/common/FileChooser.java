...
public class FileChooser {
    ...
    private class View {
        ...
        public File[] getFiles(File file) {
            ...
            for (int i = 0; i < list.length; i++) {
                ...
                if (files[i].isDirectory()) { // separate directories and files
                    String swap = list[dircount];
                    list[dircount] = list[i];
                    list[i] = swap;
                    File fwap = files[dircount];
                    files[dircount] = files[i];
                    files[i] = fwap;
                    dircount++;
                }
            }
            ...
            for (int i = 0; i < list.length; i++) { // sort names
                for (int j = i;
                        (j > 0) && ((i < dircount) || (j > dircount)) && (collator.compare(list[j - 1], list[j]) > 0);
                        j--) {
                    String swap = list[j];
                    list[j] = list[j - 1];
                    list[j - 1] = swap;
        ...
        public Image getIcon(Thinlet thinlet, File file) {
            if (file.isDirectory()) {
                if (closedicon == null) {
                    closedicon = thinlet.getIcon("/icon/folder.gif");
                }
                return closedicon;
            } else {
                if (fileicon == null) {
                    fileicon = thinlet.getIcon("/icon/new.gif");
                }
                return fileicon;
            }
        }
