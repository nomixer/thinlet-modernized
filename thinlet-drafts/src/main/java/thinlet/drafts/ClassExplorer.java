        ...
        try {
            ...
            try {
                ...
                for (int i = 0; i < constructors.length; i++) {
                    Object constructornode = createNode(
                            thinlet,
                            classnode,
                            Modifier.toString(constructors[i].getModifiers()) + " " + constructors[i].getName(),
                            classicon);
                ...
                for (int i = 0; i < fields.length; i++) {
                    Object fieldnode = createNode(
                            thinlet,
                            classnode,
                            Modifier.toString(fields[i].getModifiers()) + " " + fields[i].getName(),
                            fieldicon);
                }

                Method[] methods = aclass.getDeclaredMethods();
                for (int i = 0; i < methods.length; i++) {
                    Object methodnode = createNode(
                            thinlet,
                            classnode,
                            Modifier.toString(methods[i].getModifiers()) + " " + getNameOf(methods[i].getReturnType())
                                    + " " + methods[i].getName(),
                            methodicon);
