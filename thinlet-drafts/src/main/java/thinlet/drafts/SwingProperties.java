...
public class SwingProperties {
        ...
        for (Enumeration keys = defaults.keys(); keys.hasMoreElements(); ) {
            ...
            if (value instanceof Color) {
                ...
                thinlet.setString(
                        valuecell,
                        "text",
                        "0x" + Integer.toHexString(0xff000000 | color.getRGB()).substring(2));
            } else if (value instanceof Font) {
                Font font = (Font) value;
                StringBuffer fonttext = new StringBuffer(font.getName());
                if (font.isBold()) {
                    fonttext.append(" bold");
                }
                if (font.isItalic()) {
                    fonttext.append(" italic");
                }
                fonttext.append(' ');
                fonttext.append(font.getSize());
    ...
    private static Image createIcon(Component component, Color color) {
        ...
        for (int x = 0; x < 12; x++) {
            for (int y = 0; y < 12; y++) {
                pix[x + y * 12] = ((x > 0) && (x < 11) && (y > 0) && (y < 11)) ? rgb : 0xff666666;
            }
        }
        return component.createImage(new MemoryImageSource(12, 12, pix, 0, 12));
    }

    public static void main(String[] args) throws Exception {
        Thinlet thinlet = new Thinlet();
        thinlet.setFont(new java.awt.Font("Tahoma", java.awt.Font.PLAIN, 11));
        thinlet.setColors(0xf2f1e4, 0x000000, 0xffffff, 0x909090, 0xb0b0b0, 0xededed, 0xc7c5b2, 0xe68b2c, 0xf2c977);
